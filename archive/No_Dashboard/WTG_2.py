...
# SPs start rotated +270° at yaw=0 (per your preference)
...
# --- HUD / Stacked rows (world-anchored, billboarded) ---
show_hud           = True
hud_title          = "Blinc_iM"
hud_title_size     = 1.2    # meters
hud_text_size      = 0.7    # meters
hud_row_gap_world  = 1.0    # meters between stacked lines
...
# Show dynamic geo labels (turns OFF static SP text to avoid overlap)
...
def make_sp_geo_labels_entity(frame_id_scan: str,
    ...
    """Builds 'SP#: lat, lon' labels near SP markers in SCAN frame (so they yaw)."""
    ...
    for (px, py), name in zip(pts, names):
        ...
        pose = Pose(position=Vector3(x=lx, y=ly, z=label_height_m),
                    orientation=Quaternion(x=0,y=0,z=0,w=1))
        texts.append(_mk_text(pose, txt, text_size_m, color))
...
def make_tracker_geo_label_entity(frame_id_world: str,
    ...
    """Builds 'TRACKER: lat, lon' label next to the tracker marker in WORLD (no yaw)."""
    if not HAVE_TEXT:
    ...
    pose = Pose(position=Vector3(x=lx, y=ly, z=label_height_m),
                orientation=Quaternion(x=0,y=0,z=0,w=1))
    texts = [_mk_text(pose, txt, text_size_m, color)]
...
def build_scene_entities():
    ...
    # Add a static "TRACKER" tag only if we are NOT drawing dynamic geo labels
    if HAVE_TEXT and not use_dynamic_sp_labels:
        r = max(math.hypot(px, py), 1e-6)
        ux, uy = px / r, py / r
        lx, ly = px + ux * label_offset_m, py + uy * label_offset_m
...
def make_hud_entity(yaw_now_deg: float, rpm_now: float, rot_dir: float,
                    rotor_dia: float, hub_ht: float, yaw_rate_dps: float,
                    sp_offset_deg: float, tower_h: float,
                    entity_id: str = "hud_panel") -> SceneEntity:
    """Billboard + world-sized text, stacked vertically near top-right."""
    if not HAVE_TEXT or not show_hud:
        return SceneEntity(frame_id="world", id=entity_id, timestamp=now_ts(),
                           lifetime=Duration.from_secs(0),
                           triangles=[TriangleListPrimitive(points=[], color=Color(r=0,g=0,b=0,a=0))])

    ground_r = 1.5 * rotor_dia
    x_pos = ground_r + hud_x_offset_m   # a little outside the rotor ring
    z_pos = hub_ht + hud_z_offset_m     # just above hub height
    y_pos = 0.0

    yaw_disp = (yaw_now_deg % 360.0 + 360.0) % 360.0
    spin_str = "CCW" if rot_dir > 0 else "CW"
    lines = [
        (hud_title,                                hud_title_size),
        (f"Rotor Dia: {rotor_dia:.2f} m",          hud_text_size),
        (f"RPM: {rpm_now:.2f} ({spin_str})",       hud_text_size),
        (f"Yaw: {yaw_disp:.1f}°",                  hud_text_size),
        (f"Yaw Rate: {yaw_rate_dps:.2f} °/s",      hud_text_size),
        (f"Hub Height: {hub_ht:.2f} m",            hud_text_size),
        (f"SP Offset: {sp_offset_deg:.1f}°",       hud_text_size),
    ]

    def mk_text(pose, text, size_m):
        try:
            return TextPrimitive(
                pose=pose, text=text, font_size=size_m, color=hud_text_color,
                billboard=hud_billboard, background_color=hud_bg_color
            )
        except TypeError:
            try:
                return TextPrimitive(
                    pose=pose, text=text, font_size=size_m, color=hud_text_color,
                    billboard=hud_billboard, background=True, bg_color=hud_bg_color
                )
            except TypeError:
                return TextPrimitive(
                    pose=pose, text=text, font_size=size_m, color=hud_text_color,
    ...
    texts = []
    for i, (text, size_m) in enumerate(lines):
        pose = Pose(position=Vector3(x=x_pos, y=y_pos, z=z_pos - i * hud_row_gap_world),
                    orientation=Quaternion(x=0, y=0, z=0, w=1))
        texts.append(mk_text(pose, text, size_m))

    return SceneEntity(frame_id="world", id=entity_id,
                       timestamp=now_ts(), lifetime=Duration.from_secs(0),
                       texts=texts)
    ...
    while True:
        ...
        scene_ch.log(SceneUpdate(entities=[
            make_hud_entity(
                ...
                sp_offset_deg=sp_yaw_offset_deg,   # keep at 270.0 per your preference
                tower_h=30.0,                      # matches tower height used above
                entity_id="hud_panel"
            )
        ]))

        # Dynamic geo labels (SPs follow yaw in SCAN; TRACKER is fixed in WORLD)
        scene_ch.log(SceneUpdate(entities=[
